...
namespace Cli.Instructions.Markers;

public class CliInstructionMarkers : Dictionary<CliInstructionTokenType, CliInstructionTokenIndex>
{
    public static CliInstructionMarkers For(string terminalInput)
    {
        var characters = terminalInput.ToCharArray();
        if (characters.Length == 0)
        {
            return new CliInstructionMarkers
            {
    ...
    public CliInstructionTokens GetTokensFrom( string terminalInput)
    {
        var prefix = terminalInput.Strip(this, CliInstructionTokenType.Prefix);
        var name = terminalInput.Strip(this, CliInstructionTokenType.Name);
        var subName = terminalInput.StripOptionally(this, CliInstructionTokenType.SubName);
        var arguments = terminalInput.StripOptionally(this, CliInstructionTokenType.Arguments);
        
        return new CliInstructionTokens
        {
            { CliInstructionTokenType.Prefix, prefix },
            { CliInstructionTokenType.Name, name },
            { CliInstructionTokenType.SubName, subName },
            { CliInstructionTokenType.Arguments, arguments }
        };
    }
}